    ...
    if (!element) {
      ...
      try {
        ...
        for (const uri in data) {
          totalRefs += data[uri].length || 0;
        }

        const items: ReferenceTreeItem[] = [
          new ReferenceTreeItem(`${totalRefs} references in ${fileCount} files`, vscode.TreeItemCollapsibleState.None, 'summary'),
      ...
      try {
        ...
        console.log('🔍 ReferencesTreeProvider: Raw data keys:', Object.keys(data));
