...
export class ClickUpCodeLensDebug {
    ...
    try {
      ...
      for (const uri in data) {
        ...
        refs.forEach((ref: any, index: number) => {
          ...
          outputChannel.appendLine(`     Workspace: ${ref.workspaceFolderPath || 'Legacy (no workspace)'}`);
  ...
  debugShowWorkspaceFiltering(outputChannel: vscode.OutputChannel, data: any): void {
    ...
    outputChannel.appendLine(`Current workspace path: ${currentWorkspacePath || 'None (no workspace open)'}`);
    ...
    outputChannel.appendLine(`Before filtering: ${originalRefCount} references in ${originalFileCount} files`);
    outputChannel.appendLine(`After filtering: ${filteredRefCount} references in ${filteredFileCount} files`);
    
    if (originalRefCount !== filteredRefCount) {
      outputChannel.appendLine(`Filtered out: ${originalRefCount - filteredRefCount} references from other workspaces`);
    }
    ...
    try {
      ...
      for (const uri in data) {
        ...
        for (const ref of refs) {
          if (!currentWorkspacePath || !ref.workspaceFolderPath || ref.workspaceFolderPath === currentWorkspacePath) {
            workspaceRefs++;
          }
        }
      }
      ...
      result.then((choice) => {
        if (choice === 'Clear All References') {
          if (currentWorkspacePath) {
            this.performClearWorkspaceReferences(data, currentWorkspacePath, taskReferences, fireChangeEvent);
          } else {
  ...
  private performClearWorkspaceReferences(
    ...
    try {
      ...
      fireChangeEvent();
      vscode.window.showInformationMessage(`Cleared ${removedCount} task references from current workspace`);
    } catch (error) {
  ...
  clearCompletedReferences(fireChangeEvent: () => void): void {
    ...
    try {
      ...
      for (const uri in data) {
        ...
        for (const ref of refs) {
          ...
          if (!currentWorkspacePath || !ref.workspaceFolderPath || ref.workspaceFolderPath === currentWorkspacePath) {
            ...
            if (status.includes('complete') || status.includes('done') || status.includes('closed') || status.includes('resolved')) {
              completedRefs++;
            }
          }
        }
      ...
      // Show confirmation dialog
      const workspaceInfo = currentWorkspacePath ? ` in current workspace (${vscode.workspace.workspaceFolders?.[0]?.name || 'current project'})` : '';
      const result = vscode.window.showWarningMessage(
  ...
  private performClearCompletedReferences(data: any, currentWorkspacePath: string | undefined, fireChangeEvent: () => void): void {
    try {
      const cleanedData: any = {};
      let removedCount = 0;
      ...
      for (const uri in data) {
        ...
        for (const ref of refs) {
          const status = (ref.taskStatus?.status || ref.status || '').toLowerCase();
          const isCompleted = status.includes('complete') || status.includes('done') || status.includes('closed') || status.includes('resolved');
          
          // Keep reference if:
          // 1. Not completed, OR
          // 2. Not in current workspace (if workspace filtering is active)
          const inCurrentWorkspace = !currentWorkspacePath || !ref.workspaceFolderPath || ref.workspaceFolderPath === currentWorkspacePath;
      ...
      vscode.window.showInformationMessage(`Successfully removed ${removedCount} completed task references${workspaceInfo}.`);
    } catch (error) {
